...
void McuDnsResolver_SetDnsFoundCallback(DnsResolver_dns_found_cb_fptr callback) {
...
int McuDnsResolver_ResolveName(const char *name, McuDnsResolver_info_t *info, int32_t timeoutMs) {
  if (name[0]>='0' && name[0]<='9') { /* IP4 address? */
    ...
    memset(&info->resolved_addr, 0, sizeof(info->resolved_addr));
    #if McuLib_CONFIG_CPU_IS_RPxxxx
    ...
    /* here it should be ERR_INPROGRESS */
